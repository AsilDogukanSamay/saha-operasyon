...
# --- YARDIMCI FONKSİYON: RESİM İŞLEME ---
...
if not view_df.empty:
    ...
    with dashboard_tabs[0]:
        ...
        # Harita Legend (Görsel Açıklama)
        legend_html = ""
        if "Ziyaret" in map_view_mode:
            legend_html = """
            <div style="display:flex; gap:15px; color:#E2E8F0; font-size:13px; font-weight:600; margin-bottom:10px;">
                <div style="display:flex; align-items:center;"><span style="width:10px; height:10px; background:#10B981; border-radius:50%; margin-right:8px;"></span>Tamamlanan</div>
                <div style="display:flex; align-items:center;"><span style="width:10px; height:10px; background:#DC2626; border-radius:50%; margin-right:8px;"></span>Bekleyen</div>
                <div style="display:flex; align-items:center; border-left:1px solid #333; padding-left:15px;"><span style="width:10px; height:10px; background:#00FFFF; border-radius:50%; margin-right:8px; box-shadow:0 0 5px #00FFFF;"></span>Canlı Konum</div>
            </div>
            """
        else:
            legend_html = """
            <div style="display:flex; gap:15px; color:#E2E8F0; font-size:13px; font-weight:600; margin-bottom:10px;">
                <div style="display:flex; align-items:center;"><span style="width:10px; height:10px; background:#EF4444; border-radius:50%; margin-right:8px;"></span>Hot Lead</div>
                <div style="display:flex; align-items:center;"><span style="width:10px; height:10px; background:#F59E0B; border-radius:50%; margin-right:8px;"></span>Warm Lead</div>
                <div style="display:flex; align-items:center;"><span style="width:10px; height:10px; background:#3B82F6; border-radius:50%; margin-right:8px;"></span>Cold Lead</div>
                <div style="display:flex; align-items:center; border-left:1px solid #333; padding-left:15px;"><span style="width:10px; height:10px; background:#00FFFF; border-radius:50%; margin-right:8px; box-shadow:0 0 5px #00FFFF;"></span>Canlı Konum</div>
            </div>
            """
    ...
    with dashboard_tabs[3]:
        # Tüm klinikleri listeye al, ancak yakındakini varsayılan seç
        all_clinics = processed_df["Klinik Adı"].tolist()
        
        # Yakındakileri bul (1.5 km)
        ...
        if selected_clinic_ai:
            ...
            # Session state'den eski notu çek
            ...
            with col_close:
                st.link_button(f"✅ Ziyareti Kapat (Excel)", SOURCE_EXCEL_URL, use_container_width=True)
    ...
    if st.session_state.role == "Yönetici":
        with dashboard_tabs[4]:
            ...
            # Yönetici için Aggregation (Hatasız)
            # İsimleri boşluksuz tanımlıyoruz (Toplam_Hedef)
            perf_stats = main_operational_df.groupby("Personel").agg(
            ...
            with chart_col2:
                pie_chart = alt.Chart(main_operational_df['Lead Status'].value_counts().reset_index()).mark_arc(innerRadius=60).encode(
                    theta='count',
                    color='Lead Status',
            ...
            # Detaylı Kartlar (HTML ile Şık Görünüm)
            for index, row in perf_stats.iterrows():
                ...
                # Kart HTML'i (Hata vermemesi için string içinde tanımlandı)
                card_html_content = f"""
                <div class="admin-perf-card">
                    <div style="display:flex; justify-content:space-between; align-items:center;">
                        <div>
                            <span style="font-size:18px; font-weight:800; color:white;">{row['Personel']}</span>
                        </div>
