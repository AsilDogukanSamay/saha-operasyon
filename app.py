...
# =================================================
# 1. PREMIUM PRO CONFIG & CSS (V66 Orijinal Stil)
# =================================================
st.set_page_config(page_title="Medibulut Saha Pro V81", layout="wide", page_icon="📍")
...
def load_data(url, role):
    try:
        ...
        data.columns = [c.strip() for c in data.columns] # Sütun isimlerini temizle
...
if not df.empty:
    ...
    with tab1:
        ...
        if not d_df.empty:
            ...
            view_lat = c_lat if c_lat else d_df["lat"].mean()
            view_lon = c_lon if c_lon else d_df["lon"].mean()
            
            st.pydeck_chart(pdk.Deck(layers=layers, initial_view_state=pdk.ViewState(latitude=view_lat, longitude=view_lon, zoom=12), tooltip={"text":"{Klinik Adı}"}))
        ...
        if st.session_state.role == "Admin":
            ...
            def to_excel(df_exp):
                ...
                with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
                    df_exp.to_excel(writer, index=False, sheet_name='Saha Operasyon')
                    worksheet = writer.sheets['Saha Operasyon']
                    for i, col in enumerate(df_exp.columns):
                        column_len = max(df_exp[col].astype(str).str.len().max(), len(col)) + 2
                        worksheet.set_column(i, i, column_len)
                return output.getvalue()

            excel_data = to_excel(df)
            st.download_button(label="📊 Gerçek Excel Dosyası Olarak İndir (.xlsx)", data=excel_data, file_name=f"medibulut_rapor_{time.strftime('%d_%m_%Y')}.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
        else:
            st.warning("⚠️ Bu alan sadece yöneticiler içindir.")
else:
    st.error("❌ Veri bulunamadı. Lütfen Google Sheets'teki 'Personel' sütununda 'Doğukan' yazdığından emin ol.")
