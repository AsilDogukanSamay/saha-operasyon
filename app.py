...
import base64 # Yerel dosyayı okumak için
import os
...
# =================================================
# KANKA: İndirdiğin logonun adı tam olarak bu olmalı:
LOCAL_LOGO_FILE = "logo.png" 

# Logoyu sisteme gömmek için Base64 formatına çeviren fonksiyon
def get_base64_logo(bin_file):
    with open(bin_file, 'rb') as f:
        data = f.read()
    return base64.b64encode(data).decode()

# Logoyu hazırla (Dosya varsa kullan, yoksa yedek bulut ikonuna dön)
if os.path.exists(LOCAL_LOGO_FILE):
    logo_base64 = get_base64_logo(LOCAL_LOGO_FILE)
    APP_LOGO = f"data:image/png;base64,{logo_base64}"
    PAGE_ICON = LOCAL_LOGO_FILE
else:
    APP_LOGO = "https://medibulut.s3.eu-west-1.amazonaws.com/pages/general/logo.svg"
    PAGE_ICON = "☁️"
...
# =================================================
# 2. GİRİŞ EKRANI (BEYAZ TEMA & SENİN LOGON)
# =================================================
if not st.session_state.auth:
    ...
    with col1:
        ...
        <div style="margin-bottom: 20px; display: flex; align-items: center;">
            <img src="{APP_LOGO}" style="height: 60px; margin-right: 15px; border-radius: 12px; box-shadow: 0 4px 10px rgba(0,0,0,0.1);">
            <div>
    ...
    with col2:
        ...
        medi_logo   = APP_LOGO # KARTTA DA SENİN LOGON VAR
...
# --- VERİ BAĞLANTISI ---
SHEET_ID = "1300K6Ng941sgsiShQXML5-Wk6bR7ddrJ4mPyJNunj9o" # ÇALIŞAN ORİJİNAL ID
...
else: df = all_df[all_df["Personel"].apply(lambda x: unicodedata.normalize('NFKD', str(x)).encode('ASCII', 'ignore').decode('utf-8').lower().replace(" ", "")) == st.session_state.user.lower().replace(" ", "")]

# --- SIDEBAR ---
with st.sidebar:
    if os.path.exists(LOCAL_LOGO_FILE): st.image(LOCAL_LOGO_FILE, width=150)
...
<div style='display: flex; align-items: center; margin-bottom: 20px;'>
    <img src="{APP_LOGO}" style="height: 50px; margin-right: 15px; border-radius:10px; background:white; padding:2px;">
...
if not df.empty:
    ...
    tabs = st.tabs(["🗺️ Harita", "📋 Liste", "📍 Rota", "✅ İşlem & AI", "🏆 Analiz & Liderlik", "⚙️ Admin"]) if st.session_state.role == "Admin" else st.tabs(["🗺️ Harita", "📋 Liste", "📍 Rota", "✅ İşlem & AI"])
