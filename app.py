...
# =================================================
st.set_page_config(page_title="Medibulut Saha Pro V97", layout="wide", page_icon="🚀")
...
# SENİN İÇİN KRİTİK OLAN KOORDİNAT DÜZELTİCİ
def fix_coord(val):
    try:
...
# SKORLAMA (Yeni Eklediğimiz Özellik)
...
def load_data_v97(url):
    try:
        ...
        data["Skor"] = data.apply(calculate_score, axis=1) # Skorları hesapla
...
with st.sidebar:
    ...
    if st.button("🔄 Verileri Yenile", use_container_width=True):
        st.cache_data.clear()
        st.toast("Veriler Yenileniyor...", icon="🔄")
        time.sleep(0.5)
        st.rerun()
...
if not df.empty:
    ...
    total_score = d_df["Skor"].sum() # Toplam Puan
    
    k1, k2, k3, k4 = st.columns(4)
    k1.metric("Toplam Hedef", total)
    k2.metric("🏆 Toplam Puan", total_score) # Skor eklendi
    ...
    with t4:
        ...
        # Tüm veriden skorları hesaplayıp grupla
