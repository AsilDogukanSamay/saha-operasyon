...
if "email" not in st.session_state: st.session_state.email = None # MAİL HAFIZASI
...
# ==============================================================================
# 3. KURUMSAL GİRİŞ EKRANI (FULL - HİÇBİR EKSİK YOK)
# ==============================================================================
            ...
            if st.button("Güvenli Giriş Yap"):
                ...
                if clean_mail in db:
                    if db[clean_mail]["pass"] == u_pass:
                        ...
                        st.session_state.email = clean_mail # KRİTİK: Maili kaydettik
        ...
        <html><head><style>
            ...
            <div class="hero-card">
                ...
                <div class="product-grid">
                    <a href="https://www.dentalbulut.com" target="_blank"><div class="product-card"><div class="icon-wrapper"><img src="{dental}"></div><div><h4 style="margin:0;">Dentalbulut</h4></div></div></a>
                    <a href="https://www.medibulut.com" target="_blank"><div class="product-card"><div class="icon-wrapper"><img src="{medibulut}"></div><div><h4 style="margin:0;">Medibulut</h4></div></div></a>
                    <a href="https://www.diyetbulut.com" target="_blank"><div class="product-card"><div class="icon-wrapper"><img src="{diyet}"></div><div><h4 style="margin:0;">Diyetbulut</h4></div></div></a>
                    <a href="https://kys.medibulut.com" target="_blank"><div class="product-card"><div class="icon-wrapper"><img src="{kys}"></div><div><h4 style="margin:0;">Medibulut KYS</h4></div></div></a>
                </div>
            </div></body></html>
        """
...
# ==============================================================================
# 4. OPERASYONEL DASHBOARD (FULL TASARIM KORUNDU)
# ==============================================================================
st.markdown("""
<style>
...
# --- YENİ FİLTRELEME SİSTEMİ (MAİL EŞLEŞTİRME) ---
if st.session_state.role == "Yönetici":
    view_df = main_df
else: 
    # Personel sadece kendi "Atanan Mail"ini görür
...
with st.sidebar:
    ...
    st.caption(f"📧 {st.session_state.email}") # Maili de gösterelim
    st.divider()
...
if not view_df.empty:
    ...
    with dashboard_tabs[3]:
        ...
        if selected_clinic_ai:
            ...
            ai_msg = ""
            if "hot" in lead_stat: ai_msg = f"🔥 **KRİTİK FIRSAT:** {selected_clinic_ai} satın almaya çok yakın! Hemen demo onayı al, fiyat teklifini sun."
            elif "warm" in lead_stat: ai_msg = f"🟠 **İLGİLİ ADAY:** {selected_clinic_ai} sistemle ilgileniyor ama kararsız. Mobil uygulamanın doktorun hayatını nasıl kolaylaştıracağını anlat."
            else: ai_msg = f"🔵 **SOĞUK ADAY:** {selected_clinic_ai} henüz bizi tam tanımıyor. Zorlama yapma, sadece profesyonel bir tanışma gerçekleştir."
            ...
            if st.session_state.notes:
                st.markdown("---")
                notes_data = [{"Klinik": k, "Not": v, "Tarih": datetime.now().strftime("%Y-%m-%d %H:%M")} for k, v in st.session_state.notes.items()]
