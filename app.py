...
@st.cache_data(ttl=5)
def load_and_optimize(url, role, u_lat, u_lon):
    try:
...
df = load_and_optimize(CSV_URL, st.session_state.role, c_lat, c_lon)
...
with tab3:
    if st.session_state.role == "Admin":
        ...
        st.download_button(label="📊 Raporu Excel Olarak İndir (.xlsx)", data=excel_data, file_name=f"medibulut_saha_rapor.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
    else:
