...
if not st.session_state.auth:
    ...
    <style>
        ...
        div.stButton > button { background: linear-gradient(to right, #2563EB, #1D4ED8) !important; color: white !important; border: none; width: 100%; max-width: 350px; padding: 14px !important; border-radius: 10px; font-weight: 800; font-size: 16px; margin-top: 25px; box-shadow: 0 4px 6px -1px rgba(37, 99, 235, 0.3); transition: all 0.3s ease; }
...
with st.sidebar:
    ...
    st.markdown("""<div style='color:#2563EB; font-weight:900; font-size: 24px; text-align:center; margin-bottom:10px; font-family:"Inter";'>Saha<span style='color:#6B7280; font-weight:300;'>Bulut</span></div>""", unsafe_allow_html=True)
...
if not view_df.empty:
    ...
    with dashboard_tabs[3]:
        ...
        if selected_clinic_ai:
            ...
            with col_close: st.link_button(f"✅ Ziyareti Kapat (Excel)", EXCEL_DOWNLOAD_URL, use_container_width=True)
    ...
    if st.session_state.role == "Yönetici":
        with dashboard_tabs[4]:
            ...
            st.pydeck_chart(pdk.Deck(map_style=pdk.map_styles.CARTO_DARK, initial_view_state=pdk.ViewState(latitude=map_df["lat"].mean() if not map_df.empty else 41.0, longitude=map_df["lon"].mean() if not map_df.empty else 29.0, zoom=10), layers=[personel_layer], tooltip={"html": "<b>Klinik:</b> {Klinik Adı}<br><b>Durum:</b> {Lead Status}<br><b>Personel:</b> {Personel}"}))
            st.divider()
