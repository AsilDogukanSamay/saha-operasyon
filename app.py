...
if "email" not in st.session_state: st.session_state.email = None # MAİL HAFIZASI EKLENDİ
            ...
            if st.button("Güvenli Giriş Yap"):
                ...
                if clean_mail in db:
                    if db[clean_mail]["pass"] == u_pass:
                        ...
                        st.session_state.email = clean_mail # MAİL HAFIZAYA ALINDI
...
<style>
    ...
    .hd-sidebar-logo {
        width: 50%; border-radius: 15px; image-rendering: -webkit-optimize-contrast;
        box-shadow: 0 4px 15px rgba(0,0,0,0.4); margin-bottom: 15px; display: block;
    }
    
    /* Header Container */
    .header-master-wrapper { 
        display: flex; align-items: center; justify-content: space-between; 
        flex-wrap: wrap; gap: 20px; margin-bottom: 40px; padding-bottom: 20px; 
        border-bottom: 1px solid rgba(255,255,255,0.05); 
    }
    
    /* Canlı Konum Rozeti */
    .location-status-badge { 
        background: rgba(59, 130, 246, 0.1); color: #60A5FA; border: 1px solid #3B82F6; 
        padding: 8px 18px; border-radius: 25px; font-size: 13px; font-weight: 600; 
        font-family: 'Inter', sans-serif; white-space: nowrap; display: flex; 
    ...
    div[data-testid="stMetric"] { 
        ...
        border-radius: 16px; padding: 20px !important; border: 1px solid rgba(255, 255, 255, 0.1); 
    ...
    .map-legend-pro-container { 
        ...
        border: 1px solid rgba(255, 255, 255, 0.1); display: flex; flex-wrap: wrap; 
        gap: 25px; justify-content: center; align-items: center; margin: 0 auto; 
    ...
    /* Global Butonlar */
    div.stButton > button { 
        background-color: #238636 !important; color: white !important; border: none; 
        font-weight: 600; border-radius: 8px; 
    }
    
    a[kind="primary"] { 
        background-color: #1f6feb !important; color: white !important; text-decoration: none; 
        padding: 8px 16px; border-radius: 8px; display: block; text-align: center; font-weight: bold; 
    }
    
    /* Admin Performans Kartları */
    .admin-perf-card { 
        background: rgba(255, 255, 255, 0.03); padding: 20px; border-radius: 12px; 
        margin-bottom: 15px; border-left: 4px solid #3B82F6; border: 1px solid rgba(255, 255, 255, 0.05); 
    }
    ...
    .progress-bar-fill { 
        background: linear-gradient(90deg, #4ADE80 0%, #22C55E 100%); height: 8px; 
        border-radius: 6px; transition: width 0.5s; 
    }
    
    /* Dashboard Footer */
    .dashboard-signature { 
        text-align: center; margin-top: 60px; padding: 30px; border-top: 1px solid rgba(255, 255, 255, 0.05); 
        font-size: 12px; color: #4B5563; font-family: 'Inter', sans-serif; 
    }
...
if not view_df.empty:
    ...
    with dashboard_tabs[0]:
        ...
        layers = [
            pdk.Layer("ScatterplotLayer", data=processed_df, get_position='[lon, lat]', get_color='color', get_radius=25, radius_min_pixels=5, pickable=True)
        ]
        if user_lat:
   ...
   # --- TAB 4: İŞLEM & AI ---
    with dashboard_tabs[3]:
        ...
        if selected_clinic_ai:
            ...
            ai_msg = ""
            if "hot" in lead_stat: ai_msg = f"Kritik Fırsat! 🔥 {selected_clinic_ai} şu an 'HOT' statüsünde. Satın almaya çok yakınlar. Önerim: %10 İndirim kozunu hemen masaya koy ve satışı kapat!"
            elif "warm" in lead_stat: ai_msg = f"Dikkat! 🟠 {selected_clinic_ai} 'WARM' durumda. İlgililer ama kararsızlar. Bölgedeki diğer mutlu müşterilerimizden (referanslardan) bahsederek güven kazanabilirsin."
            else: ai_msg = f"Bilgilendirme. 🔵 {selected_clinic_ai} şu an 'COLD'. Henüz bizi tanımıyorlar. Sadece tanışma ve broşür bırakma hedefli git. Zorlama, sadece güven ver."
    ...
    if st.session_state.role == "Yönetici":
        with dashboard_tabs[4]:
            ...
            if secilen_personel == "Tüm Ekip": map_df = main_df.copy()
            else: map_df = main_df[main_df["Personel"] == secilen_personel]
            ...
            if os.path.exists(DB_FILE):
                with open(DB_FILE, "r", encoding="utf-8") as f: users_data = json.load(f)
                users_list = []
                for email, info in users_data.items():
                    users_list.append({"E-Posta": email, "İsim": info.get("name", ""), "Rol": info.get("role", ""), "Kurtarma Anahtarı": info.get("recovery_key", ""), "Şifre": info.get("pass", "")})
                df_users = pd.DataFrame(users_list)
                st.dataframe(df_users, use_container_width=True)
                buf_users = BytesIO()
                with pd.ExcelWriter(buf_users, engine='xlsxwriter') as writer: df_users.to_excel(writer, index=False)
                st.download_button(label="📥 Kullanıcı Listesini İndir", data=buf_users.getvalue(), file_name=f"Kullanicilar_{datetime.now().date()}.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
            else: st.warning("Henüz veritabanı dosyası oluşmamış.")
