...
# ==============================================================================
# 1. SAYFA KONFİGÜRASYONU (EN BAŞTA OLMALI)
# ==============================================================================
# Streamlit'te set_page_config her zaman ilk çalışan st komutu olmalıdır.
st.set_page_config(
    page_title="SahaBulut Operasyon Sistemi",
    layout="wide",
    page_icon="☁️",
...
# ==============================================================================
# 2. SİSTEM YAPILANDIRMASI VE SABİTLER
# ==============================================================================
...
try:
    # GitHub/Streamlit Cloud Secrets kontrolü
    if "GOOGLE_API_KEY" in st.secrets:
        ...
        # API Key yoksa sessizce devam et, AI özellikleri çalışmayacak
        pass
...
# Yerel Dosya Yolları (GitHub'da dosya varsa çalışır, yoksa hata vermez)
...
# ==============================================================================
# 3. YARDIMCI FONKSİYON KÜTÜPHANESİ
# ==============================================================================
...
if not st.session_state.auth:
    ...
    <style>
        ...
        div[data-testid="stTextInput"] label { color: #111827 !important; font-weight: 700 !important; font-family: 'Inter', sans-serif; font-size: 14px !important; margin-bottom: 8px !important; }
        div[data-testid="stTextInput"] input { background-color: #F9FAFB !important; color: #111827 !important; border: 1px solid #D1D5DB !important; border-radius: 10px !important; padding: 12px 15px !important; font-size: 16px !important; box-shadow: 0 1px 2px 0 rgba(0, 0, 0, 0.05); }
...
# ==============================================================================
# 5. OPERASYONEL DASHBOARD (KOYU TEMA & DETAYLI CSS)
# ==============================================================================
st.markdown("""
<style>
...
if not view_df.empty:
    ...
    with dashboard_tabs[0]:
        ...
        layers = [
            pdk.Layer(
                ...
                get_color='color', get_radius=25, radius_min_pixels=5, pickable=True
            )
        ]
        ...
        if user_lat:
            layers.append(
                pdk.Layer(
                    ...
                    get_position='[lon,lat]', get_color=[0, 255, 255],
                    get_radius=35, radius_min_pixels=7, stroked=True,
                    get_line_color=[255, 255, 255], get_line_width=20
                )
            )
    ...
    with dashboard_tabs[2]:
        ...
        st.dataframe(
            ...
            column_config={"Mesafe_km": st.column_config.NumberColumn("Mesafe (km)", format="%.2f")},
            use_container_width=True, hide_index=True
        )
    ...
    with dashboard_tabs[3]:
        ...
        lead_stat = st.selectbox("Müşteri Lead Durumu:", ["Hot", "Warm", "Cold"])
        user_context = st.text_area("Sahadan Gözlemlerin:", placeholder="Örn: Doktor sistemi beğendi ama fiyatı pahalı buldu...", height=120)

        if st.button("🚀 Strateji Üret (AI)", use_container_width=True):
            if not api_active:
                st.error("API Anahtarı bulunamadı. Lütfen yönetici ile iletişime geçin.")
                ...
                with st.spinner("AI analiz yapıyor..."):
                    try:
                        ...
                        Satılan ürünler: Dentalbulut, Medibulut, Diyetbulut
                        Lead Durumu: {lead_stat}
                        Gözlem: {user_context}
                        Görev: Personele özel 3 kısa aksiyon önerisi ver.
                        """
    ...
    if st.session_state.role == "Yönetici":
        with dashboard_tabs[4]:
            ...
            personel_layer = pdk.Layer("ScatterplotLayer", data=map_df, get_position='[lon, lat]', get_color='color', get_radius=100, radius_min_pixels=6, pickable=True)

            st.pydeck_chart(pdk.Deck(
                map_style=pdk.map_styles.CARTO_DARK,
                initial_view_state=pdk.ViewState(latitude=map_df["lat"].mean() if not map_df.empty else 41.0, longitude=map_df["lon"].mean() if not map_df.empty else 29.0, zoom=10),
            ...
            with gc1:
                bar = alt.Chart(stats).mark_bar(cornerRadiusTopLeft=10).encode(
                    x=alt.X('Personel', sort=alt.EncodingSortField(field="S_Toplam", order="descending")), 
                    y='S_Toplam', 
                    color='Personel'
                ).properties(height=350)
                st.altair_chart(bar, use_container_width=True)
            with gc2:
                pie_data = main_df['Lead Status'].value_counts().reset_index()
                pie_data.columns = ['Lead Status', 'count']
                pie = alt.Chart(pie_data).mark_arc(innerRadius=60).encode(theta='count', color='Lead Status').properties(height=350)
        ...
        with dashboard_tabs[5]:
            ...
            try:
                ...
                st.error(f"Excel modülü hatası: {e}")
    ...
    st.warning("Veriler yükleniyor veya gösterilecek veri bulunamadı...")
