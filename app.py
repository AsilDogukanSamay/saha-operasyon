...
def send_welcome_email(receiver_email, user_name, user_login, user_pass, app_url):
    ...
    <p>Medibulut Saha Operasyon Sistemi hesabınız oluşturuldu.</p>
    <div style="background: #f4f4f4; padding: 15px; border-radius: 8px;">
        <p><b>Kullanıcı Adı:</b> {user_login}</p>
        <p><b>Parola:</b> {user_pass}</p>
    </div><br>
    <a href="{app_url}" style="background: #2563EB; color: white; padding: 10px 20px; text-decoration: none; border-radius: 5px;">Sisteme Giriş Yap</a>
    </body></html>
    """
        ...
        with dashboard_tabs[6]:
                    ...
                    if st.form_submit_button("Kaydet ve Mail Gönder", type="primary", use_container_width=True):
                        if ru and rp and rn and re:
                            if add_user_to_db(ru, rp, re, rr, rn):
                                send_welcome_email(re, rn, ru, rp, app_link)
                                st.success(f"Personel eklendi!")
                            else: st.error("Hata!")
            ...
            with col_sil:
                try:
                    ...
                    kullanici_sec = st.selectbox("Silinecek Personel:", ["Seçiniz..."] + silinebilir)
                    if st.button("❌ Kalıcı Olarak Sil", use_container_width=True) and kullanici_sec != "Seçiniz...":
